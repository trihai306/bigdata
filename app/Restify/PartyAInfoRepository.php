...
class PartyAInfoRepository extends Repository
{
    ...
    {
        return [id(), field('user_id')->rules('required'), // Add this line
            field('contract_id')->rules('required'), field('account_number')->rules('required'), field('email')->rules('email'), field('bank_name')->rules('required'), field('address')->rules('required'), field('recipient_name')->rules('required'),];
    }
}
